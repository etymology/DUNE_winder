###############################################################################
# Name: S_CurveMotion.py
# Uses: S-curve motion using jerk, max acceleration, and max velocity limits.
# Date: 2016-02-05
# Author(s):
#   Andrew Que <aque@bb7.com>
# Notes:
#   Buggy.  Cannot correctly interpolate some motions correctly.  Also cannot
...
class S_CurveMotion( Motion ) :
  ...
  def compute( self, jerk, acceleration, velocity, startPosition, endPosition ):
    ...
    assert position == 0 or not jerk == 0, "Cannot move without a jerk term"
