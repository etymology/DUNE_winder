...
class SimulatedMotor :
  ...
  def __init__( self, plc, tagBase, simulationTime ) :
    ...
    self._desiredVelocityTag = plc.setupTag( tagBase + "_DATA.CommandVelocity", 0         )
    self._speedTag        = plc.setupTag( tagBase + "_SPEED", 0                        )
    self._directionTag    = plc.setupTag( tagBase + "_DIR", 0                          )
    self._positionTag     = plc.setupTag( tagBase + "_DATA.ActualPosition", 0          )
    self._velocityTag     = plc.setupTag( tagBase + "_DATA.ActualVelocity", 0          )
    self._accelerationTag = plc.setupTag( tagBase + "_DATA.ActualAcceleration", 0      )
    self._motionTag       = plc.setupTag( tagBase + "_DATA.CoordinatedMotionStatus", 0 )
    self._faultTag        = plc.setupTag( tagBase + "_DATA.ModuleFault", 0             )
