...
class Process:
    ...
    def step(self):
        ...
        """
        if (
            self.controlStateMachine.isMovementReady()
    ...
    def EOT_reset(self):
        ...
        print("Hello World!")
        self._io.plcLogic._moveType.set( self.MoveTypes.EOT_reset )
    ...
    def getG_CodeDirection(self):
        ...
        """
        return (
            self.gCodeHandler.getDirection()
            if self.gCodeHandler.isG_CodeLoaded()
            else True
        )
        ...
        if not self.controlStateMachine.isMovementReady():
            ...
            for cmd in codeLineSplit:
                if "X" in cmd and re.match(
                    ...
                    x = float(xCmd[1])
                    if re.match(
                        gxy + "|" + gxyf + "|" + gx_y + "|" + gx_yf, line
                    ):  # if G105 is used then add relative coordinate
                        x = x + xPosition
                        if x < self._transferLeft - 10 and yPosition > 1000:
                            error = (
                                "Invalid XY-axis Coordinates, forbiden area due to safety of winder head [X="
                                + str(x)
                                + " < "
                    ...
                    if (
                        x < self._limitLeft or x > self._limitRight
                    ):  # if X is exeeding safety limit
                        error = (
                ...
                if (
                    x < self._transferLeft - 10
                    and y > 1000
                    and re.match(xy + "|" + xyf + "|" + fxy, line)
                ):
                    error = (
                        "Invalid XY-axis Coordinates, forbiden area due to safety of winder head [X="
                        + str(x)
