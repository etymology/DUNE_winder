...
class Process:
    ...
    def __init__(self, io: IO_map, log: Log, configuration: Configuration, systemTime: TimeSource, machineCalibration: MachineCalibration):
        ...
        self.gCodeHandler = G_CodeHandler(
            io, self.spool, machineCalibration, self.headCompensation)
        ...
        self._transferLeft = float(
            self._machineCalibration.get("transferLeft"))
        self._transferRight = float(
            self._machineCalibration.get("transferRight"))
    ...
    def executeG_CodeLine(self, line):  # sourcery skip: low-code-quality
        ...
        if not self.controlStateMachine.isMovementReady():
            ...
            # 'X1234 Y1234','X0 Y1234'
            xy = '(\ *[X]\d{1,4}(\.\d{1,2})?\ *[Y]\d{1,4}(\.\d{1,2})?\ *$)'
            # 'G105 PX123','G105  PY123',G105  PY-12', 'G105  PX-123'
            gxy = '(\ *[G]105\ *[P][XY]-?\d{1,3}(\.\d{1,2})?\ *$)'
            # 'G105  PX123 PY123'
            gx_y = '(\ *[G]105\ *[P][X]-?\d{1,3}(\.\d{1,2})?\ *[P][Y]-?\d{1,3}(\.\d{1,2})?\ *$)'
            # 'X1234 Y1234 F123'
            xyf = '(\ *[X]\d{1,4}(\.\d{1,2})?\ *[Y]\d{1,4}(\.\d{1,2})?\ *[F]\d{1,3}\ *$)'
            # 'X1234 Y1234 F123'
            fxy = '(\ *[F]\d{1,3}\ *[X]\d{1,4}(\.\d{1,2})?\ *[Y]\d{1,4}(\.\d{1,2})?\ *$)'
            # 'G105 PX123 F12','G105 PY123 F123'
            gxyf = '(\ *[G]105\ *[P][XY]-?\d{1,3}(\.\d{1,2})?\ *[F]\d{1,3}\ *$)'
            # 'G105  PX123 PY123 F123'
            gx_yf = '(\ *[G]105\ *[P][X]-?\d{1,3}(\.\d{1,2})?\ *[P][Y]-?\d{1,3}(\.\d{1,2})?\ *[F]\d{1,3}\ *$)'
            ...
            for cmd in codeLineSplit:
                if "X" in cmd and re.match(
                    ...
                    xCmd = cmd.split("X")
                    x = float(xCmd[1])
                    # if G105 is used then add relative coordinate
    ...
    def startCalibrate(
        ...
        if not self.controlStateMachine.isMovementReady():
            ...
            self.cameraCalibration.setupCalibration(
                side, startPin, direction, maxPins)
    ...
    def getLayerPinGeometry(self):
        ...
        if self.apa != None:
            ...
            for edgeIndex in range(4):
                ...
                front[edge] = [pinFront, frontDeltaX,
                               frontDeltaY, offsetX, offsetY]
                back[edge] = [pinBack, backDeltaX,
                              backDeltaY, -offsetX, -offsetY]
                ...
                pinFront = LayerFunctions.offsetPin(
                    geometry, pinFront, frontCount)
                pinBack = LayerFunctions.offsetPin(
                    geometry, pinBack,  backCount)
                ...
                pinFront = LayerFunctions.offsetPin(
                    geometry, pinFront, geometry.directionFront)
                pinBack = LayerFunctions.offsetPin(
                    geometry, pinBack,  geometry.directionBack)
    ...
    def commitCalibration(self, side, offsetX, offsetY):
        ...
        if self.apa != None:
            ...
            cameraDataHash = self.cameraCalibration.save(
                cameraDataPath, cameraDataFile)
            calibration.save()
