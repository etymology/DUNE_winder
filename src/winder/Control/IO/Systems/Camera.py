...
class Camera:
  ...
  #---------------------------------------------------------------------
  def computePixelPer_mm( self, captureData, startX, startY, deltaX, deltaY ) :
    """
    Construct pin locations based on capture data.
    $$$DEBUG - Move to a better location.

    Args:
      captureData: Data from camera unit.
  ...
  def computePinLocations(
    ...
    Args:
      ...
      pixelsPer_mm: Pixels/mm (can use computePixelPer_mm to obtain this value).
      centerX: Center X of camera in pixels.
      centerY: Center Y of camera in pixels.
    Returns:
    ...
    for row in captureData :
      ...
      if row[ "Status" ] > 0 :
        newPin[ "x" ] = row[ "MotorX" ] + ( ( row[ "CameraX" ] - centerX ) / pixelsPer_mm )
        newPin[ "y" ] = row[ "MotorY" ] + ( ( row[ "CameraY" ] - centerY ) / pixelsPer_mm )
      else :
        newPin[ "x" ] = None
        newPin[ "y" ] = None
...
if __name__ == "__main__":
  ...
  # $$$DEBUG - This needs to move to an other unit.
  #
  ...
  # Create a simulated PLC so the camera has a PLC to reference.
  ...
  pixelsPer_mm = camera.computePixelPer_mm(
    INPUT_DATA,
    300,
    110,
    8,
  ...
  pinLocations = camera.computePinLocations(
    INPUT_DATA,
    "F1", # <- Doesn't matter.
    1,    # <- Doesn't matter.
    1000, # <- Doesn't matter.
